...
<template>
  <NuxtLayout name="public">
    <main
      class="m-auto flex min-h-[65vh] w-(--width-wide) items-center
        justify-center px-window py-lg"
    >
      <section
      ...
      >
        <GridPattern
          class="pointer-events-none absolute inset-0 -z-1 opacity-30"
        />

        <div
          v-if="status === 404"
          class="relative m-auto mb-md flex h-36 max-w-72 items-center
            justify-center"
        >
          <Icon name="missing" class="text-8xl text-accent" />
          <span class="absolute top-3 left-4 rotate-12 text-2xl text-text-3"
            ><Icon name="event"
          /></span>
          <span
            class="absolute right-4 bottom-4 -rotate-12 text-3xl text-text-3"
            ><Icon name="project"
          /></span>
          <span class="absolute top-1 right-10 text-xl text-text-3"
            ><Icon name="star"
          /></span>
        </div>
        <div
          v-else
          class="m-auto mb-md flex size-28 items-center justify-center
            rounded-full border border-border-2 bg-bg-3 text-6xl text-accent
            shadow-md"
        >
        ...
        <p class="m-auto mt-xs max-w-128 text-lg leading-relaxed text-text-2">
          {{ description }}
        </p>
        ...
        <div class="mt-md flex flex-wrap justify-center gap-xs">
          <button
            v-if="status === 403"
            type="button"
            class="cursor-pointer rounded-normal bg-accent px-md py-xs
              font-semibold text-white transition hocus:brightness-110"
          ...
          <template v-else>
            ...
            <button
              v-if="status === 404"
              type="button"
              class="cursor-pointer rounded-normal bg-bg-3 px-md py-xs
                font-semibold text-text-2 transition hocus:bg-accent/20
                hocus:text-accent"
