...
const { data: tagCount } = await useFetch<{ count: number }>(
      ...
      <Box>
        <div class="flex items-center justify-between gap-md p-md">
          <div class="flex items-center gap-sm">
            <div
              class="flex size-12 items-center justify-center rounded-normal
                bg-accent/20 text-xl text-accent transition
          ...
          <span class="text-2xl font-bold text-text-2">{{
            tagCount?.count ?? 0
          }}</span>
        </div>
      </Box>
